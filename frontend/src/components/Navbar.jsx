...
const Navbar = ({ showGetStarted = true, onOpenGetStarted }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Hide navbar when scrolling down past 80px, show when scrolling up (disabled if mobile menu is open)
  ...
  return (
    <>
      {/* Dark Blur Overlay when mobile menu is open */}
      {mobileMenuOpen && (
        <div
          className="fixed inset-0 w-screen h-screen bg-black/60 backdrop-blur-sm z-40 animate-in fade-in duration-300 md:hidden"
